    ...
    return (
        <div className="flex flex-col lg:flex-row items-center gap-4 mt-4 justify-center">
            <div className="flex items-center gap-2">
                <button onClick={decrease} disabled={quantity <= 1} className="px-3 bg-gray-300 rounded cursor-pointer">
                    -
                </button>

                <input type="number" value={quantity} onChange={handleInput} min={1} max={max} className="text-base w-16 text-center rounded px-2 border" />

                <button onClick={increase} disabled={quantity >= max} className="px-3 bg-gray-300 rounded cursor-pointer">
                    +
                </button>
            </div>
