        ...
        <div className="min-h-screen flex flex-col max-w-[1200px] m-auto px-3 lg:px-0">
            ...
            <main className="my-10">
                            ...
                            <li key={item.id} className="flex flex-col gap-4 justify-between items-center border p-4 rounded md:flex-row">
                                ...
                                <div className="flex items-center gap-4">
                                    {item.merchandise.product.featuredImage?.url && (
                                        <div className="relative w-[80px] h-[80px] p-1 rounded bg-white">
                                            <Image src={item.merchandise.product.featuredImage.url} alt={item.merchandise.product.title} width={80} height={80} className="object-contain object-center max-h-full" />
                                        </div>
                                    )}
                                    <div>
                                ...
                                <div className="flex items-center gap-8">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1} className="px-2 py-1 border rounded hover:bg-white transition cursor-pointer">
                                            -
                                        </button>
                                        <span className="mx-2">{item.quantity}</span>
                                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-2 py-1 border rounded hover:bg-white transition cursor-pointer">
                                            +
                                        </button>
                                    </div>

                                    {/* Prix ligne */}
                                    {compareAt !== null && <div className="line-through text-red mr-2">{compareAt.toFixed(2)} €</div>}
                                    <div className="font-bold">{linePrice.toFixed(2)} €</div>
                                </div>
                                {/* Supprimer */}
                                <button onClick={() => removeItem(item.id)} className="bg-red-600 text-white px-3 py-1 rounded text-lg hover:bg-red-800 transition cursor-pointer">
                                    Supprimer
                                </button>
                            </li>
                        );
                ...
                <div className="mt-4 text-right">
                    <button onClick={handleCheckout} className="px-6 py-3 bg-[#D4BFA6] text-[#6b1e1e] font-bold rounded hover:bg-[#C5A572] hover:cursor-pointer transition">
                        Valider
                    </button>
                </div>
            </main>
