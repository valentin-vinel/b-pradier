...
import { useCoffret2006 } from "@/hooks/useCoffret2006";

export default function OffreComponent() {
    const { products, loading } = useOfferProducts();
    const { product, loading: loadingCoffret, error } = useCoffret2006();
    ...
    return (
        ...
        //     <p className="text-center my-4">Prix : <span className="line-through text-red">{Number(product.variants?.edges[0].node.compareAtPriceV2?.amount).toFixed(2)} €</span> <span className="text-2xl font-bold">{Number(product.variants?.edges[0].node.priceV2.amount).toFixed(2)} €</span></p>
        ...
        <main className="mt-6 lg:mt-10">
            ...
            <div className="flex gap-6 flex-col-reverse lg:flex-row-reverse ">
                {productsWithTotal.map((product) => (
                    <div key={product.id} className="flex justify-between items-center p-4 border rounded shadow-sm m-auto">
                        <div className="w-[90px] h-[90px] p-1 rounded bg-white">
                            <Image src={product.featuredImage.url} alt={product.title} width={30} height={30} className="object-contain object-center max-h-full m-auto" />
                        </div>
                        ...
                        <div className="flex-1 ml-4">
                            ...
                            <p>Quantité : {product.quantity}</p>
                            <div className="mt-1">
                                {product.totalCompare && <span className="line-through text-red-500 mr-2">{product.totalCompare.toFixed(2)} €</span>}
                                <span className="font-bold text-lg">{product.total.toFixed(2)} €</span>
                            </div>
                        </div>
                    </div>
