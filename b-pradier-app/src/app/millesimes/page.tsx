...
export const metadata = {
    title: "Millésimes",
    description: "Parcourez l'ensemble des millésimes de Bernard Pradier, de 1988 à 2010 : une collection rare, préservée avec exigence et disponible à la vente.",
};
...
export default async function Products() {
    ...
    return (
        <div className="min-h-screen flex flex-col max-w-[1200px] m-auto px-3 lg:px-0">
            ...
            <main className="mb-6">
                <h2 className="text-center font-bold mb-2 text-3xl title-millesime">MILLÉSIMES DE NOS TROIS CUVÉES</h2>
                ...
                <div className="text-center my-8 bg-white max-w-[600px] h-[100px] m-auto rounded-full flex justify-center items-center border-3 font-bold lg:text-[26px]">
                    <Link href={"/offre"}>
                        « <em>L'horizontale Millésimes 2005</em> »<br /> « Offre en 2025... <span className="text-nowrap">20ème anniversaire</span> »
                    </Link>
                </div>
                    ...
                    {collectionsWithVariants.map((collection) => (
                        <div key={collection.title} className="flex flex-col gap-6">
                            <figure className="flex flex-col justify-center items-center relative">
                                ...
                                <h3 className={`text-3xl text-white absolute h-fit m-auto translate-y-6 w-[190px] font-bold uppercase ${collectionFonts[collection.title]}`}>{collection.title}</h3>
                            </figure>
                            ...
                            {collection.products.map((product) => (
                                <article key={product.id} className="flex flex-col gap-4 mb-6 items-center">
                                    ...
                                    <div className="flex flex-col gap-0">
                                        <p className={`text-base ${product.stock > 0 && product.stock < 6 ? "text-red-900" : ""}`}>
                                            Disponible : <span className="font-bold">{product.stock}</span>
                                        </p>

                                        {product.variantId && product.stock > 0 ? (
                                            <AddToCartButton variantId={product.variantId} />
                                        ) : (
                                            <button disabled className=" border-2 text-base cursor-not-allowed relative w-[210px]">
